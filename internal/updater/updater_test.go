...
func TestMatchAsset(t *testing.T) {
	assets := []ReleaseAsset{
		{Name: "aap_1.2.0_darwin_arm64.tar.gz", BrowserDownloadURL: "https://example.com/a"},
		{Name: "aap_1.2.0_darwin_amd64.tar.gz", BrowserDownloadURL: "https://example.com/b"},
		{Name: "aap_1.2.0_windows_amd64.tar.gz", BrowserDownloadURL: "https://example.com/c"},
		{Name: "SHA256SUMS.txt", BrowserDownloadURL: "https://example.com/s"},
		{Name: "media-dedupe-app_1.2.0_darwin_arm64.tar.gz", BrowserDownloadURL: "https://example.com/old"},
	}

	got := matchAsset(assets, "darwin", "arm64")
	if got == nil || got.Name != "aap_1.2.0_darwin_arm64.tar.gz" {
		t.Fatalf("darwin/arm64 match = %+v", got)
	}

	got = matchAsset(assets, "darwin", "amd64")
	if got == nil || got.Name != "aap_1.2.0_darwin_amd64.tar.gz" {
		t.Fatalf("darwin/amd64 match = %+v", got)
	}

	got = matchAsset(assets, "windows", "amd64")
	if got == nil || got.Name != "aap_1.2.0_windows_amd64.tar.gz" {
...
func TestMatchAssetRejectsLegacyNames(t *testing.T) {
	assets := []ReleaseAsset{
		...
		{Name: "aap_1.2.0_windows_amd64.zip"},
...
func TestCheckUpdate(t *testing.T) {
	rel := map[string]any{
		...
		"assets": []map[string]any{
			{
				"name":                 "aap_9.9.9_" + runtime.GOOS + "_" + runtime.GOARCH + ".tar.gz",
				"size":                 1234,
