...
function mockApi(): AppAPI {
  ...
  return {
    ...
    DeleteFiles: async (request) => { const paths = (request.paths as string[]) || []; const groupIds = (request.groupIds as number[]) || []; const succeeded: string[] = []; for (const gid of groupIds) { const g = mockGroups.find((x) => x.groupId === gid); if (!g) continue; const targets = paths.length ? paths : g.items.filter((x) => !x.isRecommended).map((x) => x.path); succeeded.push(...targets); g.items = g.items.filter((x) => !targets.includes(x.path)); if (g.items.length <= 1) g.status = "processed" } return { succeeded: paths.length ? paths : succeeded, failed: [], mode: String(request.mode || "recycle"), markedProcessed: groupIds.filter((gid) => mockGroups.find((x) => x.groupId === gid)?.status === "processed") } },
